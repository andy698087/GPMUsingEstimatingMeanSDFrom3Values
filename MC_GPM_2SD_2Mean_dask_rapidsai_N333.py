# This version splits 1 million simulations into 5 chunks to save memory for N=333, although it is slower than the multithreaded code without chunking. 
# It obtains results identical to at least 16 digits compared to the multithreaded code without chunking.
...
class SimulPivotMC(object):
    ...
    def main(self, method):
        ...
        print(f"try use {nsamples_pkl_dir}")
        df = pd.read_pickle(nsamples_pkl_dir)
        print("success")
        ...
        if method == 'valid': 
            ...
            df[['rSampleMeanLogScale1', 'rSampleSDLogScale1', 'rSampleMeanLogScale2', 'rSampleSDLogScale2']] = df['rSampleOfRandoms'].apply(self.Mean_SD).tolist()
            df.drop(columns=['rSampleOfRandoms'])
            print('dask')
            # put the table into dask, a progress that can parallel calculating each rows using multi-thread
            df = dd.from_pandas(df[['rSampleMeanLogScale1', 'rSampleSDLogScale1', 'rSampleMeanLogScale2', 'rSampleSDLogScale2']], npartitions=16) 
        ...
        # methods estimating means and standard deviations from medians and interquartile ranges
            ...
            df['rSampleOfRandoms'] = df['rSampleOfRandoms'].apply(lambda x: [np.exp(item) for item in x])            
        ...
        if method == 'valid': 
            df_Mean = df.apply(self.GPM_log_ratio_Mean, args=(0,1,2,3), axis=1, meta=meta)
        else:
            df_Mean = df.apply(self.GPM_log_ratio_Mean, args=(0,1,2,3), meta=meta)
        
        df_record = pd.DataFrame(df_Mean.compute().tolist(), columns=['ln_ratio_Mean', 'se_ln_ratio_Mean'])
        ...
        if method == 'valid': 
            df_SD = df.apply(self.GPM_log_ratio_SD, args=(0,1,2,3), axis=1, meta=meta)        
        else:
...
method_list = ['Luo_Wan', 'qe', 'bc', 'mln', 'valid']

if __name__ == '__main__':
    # Number of Monte Carlo simulations: 200,000 simulations * 5 chunks = 1 million simulations.
    nMonteSim = 200000
    # Sample size, we choose 15, 27, 51, notation "n" in the manuscript
    for N in [333]:
        ...
        for chunk in ['1', '2', '3', '4', '5']:
            ...
            folder = f"MeanSD_From3ValuesInRaw_BCQEMLN_nSim20W{chunk}_N{N}_commonCSV"
            os.makedirs(folder, exist_ok = True)

            # Check if files from a previous run exist; continue from where the previous run left off.
            done_list = []
            ...
            for CV in [0.15, 0.3, 0.5]:
                # For each chunk, the seed_value starts from 12181988 + (simulations per chunk) * (chunk - 1), ensuring the continuation of each chunk.
                seed_value = 12181988 + nMonteSim * (int(chunk)-1)
                # the pre-determined list of seeds, using number of nMonte
                list_seeds = [i for i in range(seed_value, seed_value + nMonteSim)] 
                # put the list of seeds into a table (a.k.a DataFrame) with one column named "Seeds"  
                df_glob = pd.DataFrame({'Seeds':list_seeds})

                print(f'Samples_normal_nMonte_{nMonteSim}_N_{N}_CV_{CV} chunk={chunk}')

                nsample_pkl_name = f"Samples_normal_nMonte_{nMonteSim}_N_{N}_CV_{CV}.pkl"
                nsamples_pkl_dir = os.path.join(folder,nsample_pkl_name)
                if nsample_pkl_name in os.listdir(folder):
                    print(f"{nsample_pkl_name} exists")
                    
                else:
                    # generate log-normal distributed numbers in log scale, using mean of rMeanLogScale and standard deviation of rSDLogScale
                    df_glob['rSampleOfRandoms'] = df_glob.apply(SimulPivotMC(nMonteSim, N, CV).Samples_normal, args=('Seeds',), axis=1) 
                    
                    # save generated numbers to nsamples_pkl_dir directory
                    df_glob.to_pickle(nsamples_pkl_dir)
                    print(f"save to {nsamples_pkl_dir}")
                    
                    # delete generated numbers to free up memory
                    del df_glob
                    print("del df_glob")
